...
for (const lang of Object.keys(data)) {
    ...
    // Inject root objects (at the end of `imageRestorer: { ... }`)
    // Because inside `imageRestorer` we have a `guide` object and it ends with `}`.
    // It is safer to find the text "upscaleX4: '.*?'" then find the first `}` after `faq:` block, and append.
    // Wait, let's just match the `imageRestorer: { ... }` block to avoid complexity.
    // In our `TranslationSchema` it looks exactly like this for each lang.
    // Let's replace the EXACT end of `imageRestorer` block.
    // It's `        }\n    },`
    let foundImgResEnd = false;
    content = content.replace(
        new RegExp(`(${lang}:\\s*\\{[\\s\\S]*?imageRestorer:\\s*\\{[\\s\\S]*?faq:[\\s\\S]*?\\]\\s*\\}\\s*\\})`),
        (match, group1) => {
            foundImgResEnd = true;
