...
const indentStr = (obj, pIndent) => JSON.stringify(obj, null, 4).replace(/"([^"]+)":/g, '$1:').replace(/"/g, "'").split('\n').map((l, i) => i === 0 ? l : ' '.repeat(pIndent) + l).join('\n');
...
for (const l of langs) {
    ...
    // We match the specific end structure `    },` that follows `imageRestorer:` block.
    content = content.replace(
        new RegExp(`(^\\s{4}'?${l}'?: \\{[\\s\\S]*?imageRestorer: \\{[\\s\\S]*?faq:[\\s\\S]*?\\]\\s*\\}(\\s*);?\\s*\\})(\\s*\\},)`, 'm'),
        (match, prefix, s1, suffix) => {
            const srStr = indentStr(d.screenRecObj, 8);
            const rcStr = indentStr(d.revCalcObj, 8);
            const mmStr = indentStr(d.megaMenuObj, 8);
            return prefix + `,\n        screenRecorder: ${srStr},\n        revenueCalc: ${rcStr},\n        megaMenu: ${mmStr}\n` + suffix.replace(/^\\s+/, '    ');
        }
    );
}
