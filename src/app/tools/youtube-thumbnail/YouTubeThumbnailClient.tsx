...
import { Download, Link as LinkIcon, Loader2, Share2, Sparkles } from 'lucide-react'
...
export default function YouTubeThumbnailClient() {
    ...
    const handleGrab = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            if (id) setVideoId(id)
            else setError(t.ytGrab.invalidUrl)
            setLoading(false)
        }, 600)
    }
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-12">
            <div className="text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <p className="text-muted-foreground text-lg max-w-2xl mx-auto">{t.ytGrab.desc}</p>
            </div>
            ...
            <div className="glass-card p-8 rounded-3xl animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
                <form onSubmit={handleGrab} className="flex gap-4 flex-col sm:flex-row items-center">
                    <div className="relative flex-1 w-full">
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground"><LinkIcon className="h-5 w-5" /></div>
                        <input
                            type="text" placeholder={t.ytGrab.placeholder}
                            className="w-full pl-12 h-14 rounded-xl border-none bg-zinc-100/50 dark:bg-zinc-800/50 px-4 text-lg focus:ring-2 focus:ring-primary/20 transition-all"
                            value={url} onChange={(e) => setUrl(e.target.value)}
                        />
                    </div>
                    <button
                        type="submit" disabled={loading || !url}
                        className="h-14 px-8 rounded-xl bg-gradient-to-r from-red-600 to-orange-600 text-white font-bold text-lg hover:opacity-90 transition-all shadow-lg min-w-[160px]"
                    >
                        {loading ? <Loader2 className="animate-spin h-5 w-5" /> : <span>{t.ytGrab.getBtn}</span>}
                    </button>
                </form>
                {error && <div className="mt-4 p-4 rounded-xl bg-red-500/10 text-red-600 border border-red-500/20 text-center font-medium">{error}</div>}
            </div>
            ...
            {videoId && !loading && (
                <div key={videoId} className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    <div className="rounded-3xl overflow-hidden shadow-2xl ring-1 ring-zinc-900/10 dark:ring-white/10 group relative aspect-video bg-black">
                        <img src={`https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`} alt="Thumbnail" className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-4 backdrop-blur-sm">
                            <button
                                ...
                                className="bg-white text-black px-8 py-4 rounded-full font-bold flex items-center gap-3 hover:scale-110 shadow-xl"
                            >
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    params.set('image', `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`)
                                    window.location.href = `${rootUrl}?${params.toString()}`
                                }}
                                className="bg-primary text-white w-[240px] py-4 rounded-full font-bold flex items-center justify-center gap-3 hover:scale-110 shadow-xl"
                            >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {['hqdefault', 'mqdefault', 'sddefault'].map((quality) => (
                            <div key={quality} className="relative group rounded-xl overflow-hidden aspect-video border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800">
                                <img src={`https://img.youtube.com/vi/${videoId}/${quality}.jpg`} alt={quality} className="w-full h-full object-cover opacity-80 group-hover:opacity-100" />
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40">
                                    <button onClick={() => downloadImage(`https://img.youtube.com/vi/${videoId}/${quality}.jpg`, `thumbnail-${videoId}-${quality}.jpg`)} className="p-2 bg-white rounded-full text-black hover:scale-110 transition-transform"><Download className="h-4 w-4" /></button>
                                </div>
                            </div>
                        ))}
                    </div>
