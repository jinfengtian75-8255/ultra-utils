...
Generated by UltraUtils AI
    ...
    const generatedContent = useMemo(() => {
        ...
        const scriptIntros = {
            beginner: `안녕하세요! 오늘은 '${topic}'에 아예 입문하시는 분들을 위해 하나부터 열까지 쉽게 설명해 드릴게요.`,
            general: `안녕하세요! 요즘 핫한 '${topic}', 과연 왜 화제일까요? 오늘 영상에서 완벽하게 파헤쳐 드립니다.`,
            expert: `반갑습니다. 오늘 영상에서는 '${topic}'의 고도화된 메커니즘과 현업에서 적용 가능한 실무 테크닉을 다룹니다.`
        };
        ...
        return {
            ...
            script: `[인트로: 15초]\n${currentIntro}\n\n[본론: 3분]\n1. ${topic}의 핵심 개념 (알고 계셨나요?)\n2. ${tone === 'funny' ? '병맛 섞인' : tone === 'professional' ? '전문적인' : '꼼꼼한'} 실전 활용 사례\n3. ${target === 'beginner' ? '초보자만 아는 실수' : '고수가 알려주는 팁'} 3가지\n\n[아웃트로: 30초]\n오늘 영상도 끝까지 시청해주셔서 감사합니다. ${topic} 관련 질문은 댓글로 부탁드려요!`,
            strategy: `📍 ${topic} 노출 전략:\n1. 썸네일 포인트: ${tone === 'funny' ? '표정이 화려한 썸네일' : '깔끔하고 가독성 좋은 폰트'}와 '${topic}' 키워드 강조\n2. 시청 대상 [${target}]이(가) 자주 검색할만한 키워드를 설명 상단에 3번 반복 노출\n3. ${videoType === 'shorts' ? '루프(반복)' : '초반 15초 '} 이탈 방지 장치 마련`
        };
                        ...
                        <div className="overflow-hidden rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl bg-zinc-950 border border-white/10 flex flex-col min-h-[500px] sm:min-h-[640px]">
                            ...
                            <div className="p-8 bg-zinc-900/50 border-t border-white/5 mt-auto">
                                ...
                                <button
                                    ...
                                    disabled={!showResults}
                                    className={cn(
                                        "w-full mt-3 py-4 rounded-2xl font-bold text-base flex items-center justify-center gap-2 transition-all bg-zinc-800 text-zinc-300 hover:bg-zinc-700",
                                        !showResults && "opacity-50 cursor-not-allowed"
                                    )}
                                >
                                    <Download className="w-5 h-5" /> Download .TXT Plan
                                </button>
                            </div>
                        </div>
                    </div>
